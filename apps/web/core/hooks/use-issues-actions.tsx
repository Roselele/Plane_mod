...
export interface IssueActions {
  ...
  hideIssue?: (projectId: string | undefined | null, issueId: string) => Promise<void>;
  unhideIssue?: (projectId: string | undefined | null, issueId: string) => Promise<void>;
  updateFilters: (
    projectId: string,
...
const useProjectIssueActions = () => {
  ...
  const hideIssue = useCallback(
    async (projectId: string | undefined | null, issueId: string) => {
      if (!workspaceSlug || !projectId) return;
      return await issues.issueHide(workspaceSlug, projectId, issueId);
    },
    [issues.issueHide, workspaceSlug]
  );
  const unhideIssue = useCallback(
    async (projectId: string | undefined | null, issueId: string) => {
      if (!workspaceSlug || !projectId) return;
      return await issues.issueUnhide(workspaceSlug, projectId, issueId);
    },
    [issues.issueUnhide, workspaceSlug]
  );
  ...
  return useMemo(
    ...
    [fetchIssues, fetchNextIssues, createIssue, quickAddIssue, updateIssue, removeIssue, archiveIssue, hideIssue, unhideIssue, updateFilters]
  );
};
