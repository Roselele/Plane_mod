...
export interface IBaseIssuesStore {
  ...
  issueHide: (workspaceSlug: string, projectId: string, issueId: string) => Promise<void>;
  issueUnhide: (workspaceSlug: string, projectId: string, issueId: string) => Promise<void>;
...
export abstract class BaseIssuesStore implements IBaseIssuesStore {
  ...
  constructor(
    ...
    makeObservable(this, {
      ...
      issueHide: action.bound,
      issueUnhide: action.bound,
  ...
  /**
   * Hide an issue for the current user (personal-level hiding)
   */
  async issueHide(workspaceSlug: string, projectId: string, issueId: string) {
    // Optimistically update is_hidden flag and remove from list
    runInAction(() => {
      this.rootIssueStore.issues.updateIssue(issueId, { is_hidden: true } as any);
      this.removeIssueFromList(issueId);
    });
    try {
      await this.issueService.hideIssue(workspaceSlug, projectId, issueId);
    } catch (error) {
      // Re-add to list and revert is_hidden on error
      runInAction(() => {
        this.rootIssueStore.issues.updateIssue(issueId, { is_hidden: false } as any);
   ...
   */
  async issueUnhide(workspaceSlug: string, projectId: string, issueId: string) {
    try {
      await this.issueService.unhideIssue(workspaceSlug, projectId, issueId);
      // Update is_hidden flag so the menu switches back to "Hide"
      // Do NOT remove from list — the issue is now a normal visible work item
      runInAction(() => {
        this.rootIssueStore.issues.updateIssue(issueId, { is_hidden: false } as any);
      });
    } catch (error) {
