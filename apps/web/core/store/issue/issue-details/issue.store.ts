...
export class IssueStore implements IIssueStore {
  ...
  addIssueToStore = (issue: TIssue) => {
    const issuePayload: TIssue = {
      ...
      assignee_roles: issue?.assignee_roles,
      ...
      okr: issue?.okr,
      phase_goal: issue?.phase_goal,
      progress: issue?.progress,
      version_name: issue?.version_name,
      risk: issue?.risk,
      milestone: issue?.milestone,
      // Part 3: 子需求级商务信息
      demand_background: issue?.demand_background,
      key_notes: issue?.key_notes,
      conclusion: issue?.conclusion,
      is_privatized: issue?.is_privatized,
      lanes_count: issue?.lanes_count,
      estimated_revenue: issue?.estimated_revenue,
      estimated_profit: issue?.estimated_profit,
      settlement_status: issue?.settlement_status,
      // Part 4: 工作项级字段
      deliverables: issue?.deliverables,
      remark: issue?.remark,
      task_log: issue?.task_log,
      // Part 5: 是否多阶段
      has_phases: issue?.has_phases,
    };
