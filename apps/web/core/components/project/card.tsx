...
// ---- Category color mapping (Morandi / Macaron palette) ----
const CATEGORY_STYLE_MAP: Record<string, { backgroundColor: string; color: string }> = {
  ...
  "专项": { backgroundColor: "#F5E3D3", color: "#B06D3A" }, // 烦恼橙
};
...
export const ProjectCard = observer(function ProjectCard(props: Props) {
  ...
  const { getIndustryById } = useIndustry();
  ...
  // Compute default tab URL (respects user preferences, falls back to "info")
  ...
  const { tabPreferences } = useTabPreferences(workspaceSlug.toString(), project.id);
  const defaultTabKey = availableTabKeys.includes(tabPreferences.defaultTab)
    ? tabPreferences.defaultTab
    : DEFAULT_TAB_KEY;
  const defaultTabUrl = getTabUrl(workspaceSlug.toString(), project.id, defaultTabKey);

  const projectLink = defaultTabUrl.replace(/^\//, "");
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={cn("flex w-full flex-col gap-2 rounded-b-sm p-4", {
          ...
          <div className="flex flex-wrap gap-1.5">
            {project.category && (
              <span
                ...
                style={CATEGORY_STYLE_MAP[project.category] ?? DEFAULT_CATEGORY_STYLE}
              >
                {project.category}
              </span>
            )}
            {industryName && (
              <span className="rounded bg-purple-500/10 px-2 py-0.5 text-11 font-medium text-purple-600">
                {industryName}
              </span>
            )}
            {[...new Set(project.work_categories ?? [])].map((wc) => (
              <span key={wc} className="rounded bg-green-500/10 px-2 py-0.5 text-11 font-medium text-green-600">
                {wc}
              </span>
            ))}
            {[...new Set(project.okrs ?? [])].map((o) => (
              <span key={o} className="rounded bg-amber-500/10 px-2 py-0.5 text-11 font-medium text-amber-600">
                {o}
              </span>
            ))}
          </div>
