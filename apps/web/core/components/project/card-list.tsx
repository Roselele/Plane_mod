...
export const ProjectCardList = observer(function ProjectCardList(props: TProjectCardListProps) {
  ...
  const { fetched: industriesFetched, fetchIndustries } = useIndustry();
  const { currentWorkspaceDisplayFilters, currentWorkspaceFilters } = useProjectFilter();
  const { allowPermissions } = useUserPermissions();

  // fetch industries for card display
  useEffect(() => {
    if (workspaceSlug && !industriesFetched) {
      fetchIndustries(workspaceSlug.toString());
    }
  }, [workspaceSlug, industriesFetched, fetchIndustries]);
