...
export const IssueDetailWidgetModals = observer(function IssueDetailWidgetModals(props: Props) {
  ...
  // helpers
  // When creating a sub-work-item, default start_date to the target_date of the
  // last sibling sub-work-item that has a target_date set.
  const parentIssueIdForCreate = issueCrudOperationState?.create?.parentIssueId ?? null;
  let defaultStartDate: string | null = null;
  if (parentIssueIdForCreate) {
    const siblingIds = subIssuesByIssueId(parentIssueIdForCreate) ?? [];
    // Iterate in reverse to find the last sibling with a target_date
    for (let i = siblingIds.length - 1; i >= 0; i--) {
      const sibling = getIssueById(siblingIds[i]);
      if (sibling?.target_date) {
        defaultStartDate = sibling.target_date;
        break;
      }
    }
  }
  ...
  const createUpdateModalData: Partial<TIssue> = {
    parent_id: parentIssueIdForCreate,
