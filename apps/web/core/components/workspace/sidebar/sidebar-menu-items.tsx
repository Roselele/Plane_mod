          ...
          {isWorkspaceMenuOpen && (
            <Disclosure.Panel as="div" className="flex flex-col gap-0.5" static>
              <>
                ...
                {/* 成员状态 */}
                <Link
                  href={`/${workspaceSlug}/member-status`}
                  onClick={() => {
                    if (window.innerWidth < 768) toggleSidebar();
                    if (isExtendedSidebarOpened) toggleExtendedSidebar(false);
                  }}
                >
                  <SidebarNavItem isActive={pathname.includes("/member-status")}>
                    <div className="flex items-center gap-1.5 py-[1px]">
                      <CalendarRange className="size-4 flex-shrink-0" />
                      <p className="text-13 leading-5 font-medium">成员状态</p>
                    </div>
                  </SidebarNavItem>
                </Link>
