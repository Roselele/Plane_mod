...
class Issue(ChangeTrackerMixin, ProjectBaseModel):
    ...
    work_category = models.CharField(max_length=50, choices=WORK_CATEGORY_CHOICES, null=True, blank=True, verbose_name="工作分类")
    okr = models.CharField(max_length=50, choices=OKR_CHOICES, null=True, blank=True, verbose_name="OKR")
    phase_goal = models.TextField(null=True, blank=True, verbose_name="阶段目标")
    progress = models.IntegerField(
        default=0,
        validators=[MinValueValidator(0), MaxValueValidator(100)],
        verbose_name="完成百分比",
    )
    version_name = models.CharField(max_length=255, null=True, blank=True, verbose_name="项目版本名称")
    risk = models.TextField(null=True, blank=True, verbose_name="风险")
    ...
    # === Part 3: 子需求级商务信息（可选，多个子需求时各自填写） ===
    demand_background = models.TextField(null=True, blank=True, verbose_name="需求背景/简述")
    key_notes = models.TextField(null=True, blank=True, verbose_name="重点信息备注")
    conclusion = models.TextField(null=True, blank=True, verbose_name="结论情况")
    is_privatized = models.CharField(max_length=10, choices=IS_PRIVATIZED_CHOICES, null=True, blank=True, verbose_name="是否私有化")
    lanes_count = models.FloatField(null=True, blank=True, verbose_name="路数（万路）")
    estimated_revenue = models.DecimalField(max_digits=14, decimal_places=2, null=True, blank=True, verbose_name="预估收入")
    estimated_profit = models.DecimalField(max_digits=14, decimal_places=2, null=True, blank=True, verbose_name="预估利润")
    settlement_status = models.CharField(max_length=20, choices=SETTLEMENT_STATUS_CHOICES, null=True, blank=True, verbose_name="结算情况")

    # === Part 4: 工作项级字段（仅子工作项显示） ===
    deliverables = models.TextField(null=True, blank=True, verbose_name="输出物")
    remark = models.TextField(null=True, blank=True, verbose_name="备注")

    # === Part 4.5: 阶段级字段（仅阶段工作项显示） ===
    task_log = models.TextField(null=True, blank=True, verbose_name="任务日志")

    # === Part 5: 是否多阶段（仅子需求级使用） ===
    has_phases = models.BooleanField(default=False, verbose_name="是否多阶段")
...
class IssueHidden(ProjectBaseModel):
    """Track issues hidden by a specific user (personal-level hiding)."""
    issue = models.ForeignKey(Issue, on_delete=models.CASCADE, related_name="issue_hidden")
    ...
    def __str__(self):
        return f"{self.issue.name} hidden by {self.user.email}"
