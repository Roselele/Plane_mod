...
class IssueViewSet(BaseViewSet):
    ...
    def apply_annotations(self, issues):
        issues = (
            ...
            .annotate(
                is_hidden=Exists(
                    IssueHidden.objects.filter(issue_id=OuterRef("pk"), user=self.request.user)
                )
            )
        )
    ...
    def list(self, request, slug, project_id):
        ...
        # Exclude issues hidden by the current user (unless show_hidden=true)
        show_hidden = request.GET.get("show_hidden", "false")
        if show_hidden != "true":
            hidden_issue_ids = IssueHidden.objects.filter(
                user=request.user, project_id=project_id
            ).values_list("issue_id", flat=True)
            issue_queryset = issue_queryset.exclude(id__in=hidden_issue_ids)
...
class IssuePaginatedViewSet(BaseViewSet):
    ...
    def list(self, request, slug, project_id):
        ...
        # Annotate is_hidden for the current user
        hidden_subquery = IssueHidden.objects.filter(
            issue_id=OuterRef("pk"), user=request.user
        )
        base_queryset = base_queryset.annotate(is_hidden=Exists(hidden_subquery))
        ...
        if show_hidden != "true":
            ...
            base_queryset = base_queryset.exclude(id__in=hidden_issue_ids)

        base_queryset = base_queryset.order_by("updated_at")
        queryset = self.get_queryset().order_by("updated_at")

        # Always annotate is_hidden on queryset (needed for required_fields)
        queryset = queryset.annotate(is_hidden=Exists(hidden_subquery))

        # Exclude hidden issues from queryset too
        if show_hidden != "true":
            hidden_issue_ids = IssueHidden.objects.filter(
                user=request.user, project_id=project_id
            ).values_list("issue_id", flat=True)
            queryset = queryset.exclude(id__in=hidden_issue_ids)
...
class IssueDetailEndpoint(BaseAPIView):
    ...
    def get(self, request, slug, project_id):
        ...
        issue = issue.annotate(is_hidden=Exists(hidden_subquery))
        ...
        if show_hidden != "true":
            ...
            issue = issue.exclude(id__in=hidden_issue_ids)
