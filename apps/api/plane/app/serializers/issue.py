...
class IssueCreateSerializer(BaseSerializer):
    ...
    # 负责人分工映射：{ user_id: "产品" | "研发" | null }，按负责人维度
    assignee_roles = serializers.DictField(child=serializers.CharField(allow_blank=True, allow_null=True), required=False)
    ...
    def validate(self, attrs):
        ...
        # Validate assignees: 放开成员限制，任意已注册用户均可作为负责人
        if attrs.get("assignee_ids", []):
            attrs["assignee_ids"] = [
                user.pk
                for user in attrs["assignee_ids"]
                if isinstance(user, User)
            ]

        # Validate assignee_roles values if provided
        if attrs.get("assignee_roles"):
            allowed_roles = {"产品", "研发", None, ""}
            for user_id, role in attrs["assignee_roles"].items():
                if role not in allowed_roles:
                    raise serializers.ValidationError(
                        {"assignee_roles": f"分工 {role} 不合法，仅支持 产品 / 研发 / 无"}
                    )
    ...
    def create(self, validated_data):
        ...
        assignee_roles = validated_data.pop("assignee_roles", None) or {}
                ...
                IssueAssignee.objects.bulk_create(
                    [
                        IssueAssignee(
                            ...
                            assignee_role=assignee_roles.get(str(assignee_id)) or None,
                            issue=issue,
    ...
    def update(self, instance, validated_data):
        ...
        if assignees is not None:
            ...
            # 仅更新分工角色，不改变负责人列表
            for assignee in IssueAssignee.objects.filter(issue=instance):
                role = assignee_roles.get(str(assignee.assignee_id))
                if role in {"产品", "研发"} or role is None:
                    assignee.assignee_role = role
                    assignee.save(update_fields=["assignee_role"])
...
class IssueSerializer(DynamicBaseSerializer):
    ...
    # Assignee roles (产品 / 研发 / null) keyed by user id
    assignee_roles = serializers.SerializerMethodField()

    # Hidden status for current user (annotated on queryset)
    ...
    def get_assignee_roles(self, obj):
        return {
            str(assignee.assignee_id): assignee.assignee_role
            for assignee in obj.issue_assignee.all()
        }

    def get_is_hidden(self, obj):
        return getattr(obj, "is_hidden", False)
    ...
    class Meta:
        ...
        fields = [
            ...
            "task_log",
            "has_phases",
            # Hidden status for current user
            "is_hidden",
        ]
        read_only_fields = fields
...
class IssueListDetailSerializer(serializers.Serializer):
    ...
    def to_representation(self, instance):
        data = {
            ...
            "assignee_roles": self.get_assignee_roles(instance),
            "sub_issues_count": instance.sub_issues_count,
            "attachment_count": instance.attachment_count,
            "link_count": instance.link_count,
            # === 子需求级字段 ===
            "work_category": instance.work_category,
            "okr": instance.okr,
            "phase_goal": instance.phase_goal,
            "progress": instance.progress,
            "version_name": instance.version_name,
            "risk": instance.risk,
            "milestone": instance.milestone_id,
            "demand_background": instance.demand_background,
            "key_notes": instance.key_notes,
            "conclusion": instance.conclusion,
            "is_privatized": instance.is_privatized,
            "lanes_count": instance.lanes_count,
            "estimated_revenue": instance.estimated_revenue,
            "estimated_profit": instance.estimated_profit,
            "settlement_status": instance.settlement_status,
            "deliverables": instance.deliverables,
            "remark": instance.remark,
            "task_log": instance.task_log,
            "has_phases": instance.has_phases,
            # Hidden status for current user
            "is_hidden": getattr(instance, "is_hidden", False),
        }
...
class IssueHiddenSerializer(BaseSerializer):
    class Meta:
        model = IssueHidden
        fields = "__all__"
        read_only_fields = ["workspace", "project", "issue", "user"]
