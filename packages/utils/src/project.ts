...
export const shouldFilterProject = (
  ...
  Object.keys(filters).forEach((key) => {
    ...
    if (filterKey === "category" && filters.category && filters.category.length > 0)
      fallsInFilters = fallsInFilters && !!project.category && filters.category.includes(project.category);
    if (filterKey === "industry" && filters.industry && filters.industry.length > 0)
      fallsInFilters = fallsInFilters && !!project.industry && filters.industry.includes(project.industry);
    if (filterKey === "work_category" && filters.work_category && filters.work_category.length > 0) {
      const cats = project.work_categories ?? (project.work_category ? [project.work_category] : []);
      fallsInFilters = fallsInFilters && filters.work_category.some((c) => cats.includes(c));
    }
    if (filterKey === "okr" && filters.okr && filters.okr.length > 0) {
      const okrs = project.okrs ?? (project.okr ? [project.okr] : []);
      fallsInFilters = fallsInFilters && filters.okr.some((o) => okrs.includes(o));
    }
  });
